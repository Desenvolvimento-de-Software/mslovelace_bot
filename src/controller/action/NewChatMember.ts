...
export default class NewChatMember extends Action {
    ...
    public async run(payload: Record<string, any>): Promise<void> {
        ...
        const user = await UserHelper.getUserByTelegramId(payload.message.new_chat_member.id);
        if (!user) {
            return;
        }
        ...
        text = text.replace("{username}", payload.message.new_chat_member.first_name || payload.message.new_chat_member.username);
